...
public class PlaybackService extends Service {
	...
	public void createLocalStream(final double _position){
	   ...
	   String decodedData = "";
       savedData.edit().putString("savedSongPosition", String.valueOf((long)(_position))).commit();
		...
		mp.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
			...
			public void onCompletion(MediaPlayer mp) {
	            try {
		            savedData.edit().putString("savedSongPosition", String.valueOf(_position + 1)).commit();
       ...
       try {
	       ...
	       if( album_art != null ){
		       ...
		       Glide.with(getApplicationContext()).asBitmap().load(bitmapArt).into(PlayerActivity.albumArt);
		       Glide.with(getApplicationContext()).asBitmap().load(bitmapArt).into(PlayerActivity.miniplayerAlbumArt);
	       } else { 
		       Glide.with(getApplicationContext()).asBitmap().load(R.drawable.album_art).into(PlayerActivity.albumArt);
		       Glide.with(getApplicationContext()).asBitmap().load(R.drawable.album_art).into(PlayerActivity.miniplayerAlbumArt);
	       }
       } catch (Exception e) {
